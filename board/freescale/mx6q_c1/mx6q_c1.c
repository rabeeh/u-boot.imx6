 ...
 * Based on sabresd board code from Freescale Semiconductor, Inc.
 *
...
*could not be power off anymore*/
	...
	/* board id for linux */
	gd->bd->bi_arch_number = MACH_TYPE_C1;
...
int mx6_rgmii_rework(char *devname, int phy_addr)
{
	...
	unsigned int sig;
	phy_read(devname, phy_addr, PHY_PHYIDR1, &val);
	phy_read(devname, phy_addr, PHY_PHYIDR2, &val1);
	sig = val1 | (val << 16);
	if (sig == 0x004dd076) return 0; /* This is AR8030 RMII phy */
...
#define MX6DL_ENET_PAD_CTRL_22K (PAD_CTL_PKE | PAD_CTL_PUE  |		\
...
#elif defined CONFIG_MX6DL /* RMII1 with PLL - 0010 */
iomux_v3_cfg_t enet_pads[] = {
	MX6DL_PAD_ENET_MDIO__ENET_MDIO,
	MX6DL_PAD_ENET_MDC__ENET_MDC,
	/* AR8035 */
//	MX6DL_PAD_RGMII_TXC__ENET_RGMII_TXC, This is dangerous since it's input in AR8030 and output in AR8035
...
//	MX6DL_PAD_RGMII_RXC__ENET_RGMII_RXC,
	/* MX6DL_PAD_RGMII_RD0__ENET_RGMII_RD0 but with pull down - phy addr */
	IOMUX_PAD(0x0694, 0x02AC, 1, 0x0818, 1, MX6DL_ENET_PAD_CTRL),
	/* MX6DL_PAD_RGMII_RD1__ENET_RGMII_RD1 but with pull down - phy addr */
	IOMUX_PAD(0x0698, 0x02B0, 1, 0x081C, 1, MX6DL_ENET_PAD_CTRL_PD),
	/* MX6DL_PAD_RGMII_RD2__ENET_RGMII_RD2 but with pull down */
//	MX6DL_PAD_RGMII_RD2__ENET_RGMII_RD2,
// Remove above and return this
// After trying this; look reset strap 0010 and 0000 both works (RMII mode 1 PLL EN or DIS)
//	IOMUX_PAD(0x069C, 0x02B4, 1, 0x0820, 1, MX6DL_ENET_PAD_CTRL_PD),
	/* Following is RD2 with 22kOhm pull up */
	IOMUX_PAD(0x069C, 0x02B4, 1, 0x0820, 1, MX6DL_ENET_PAD_CTRL_22K),

	/* MX6DL_PAD_RGMII_RD3__ENET_RGMII_RD3 but with pull down */
	IOMUX_PAD(0x06A0, 0x02B8, 1, 0x0824, 1, MX6DL_ENET_PAD_CTRL_PD),
	/* MX6DL_PAD_RGMII_RX_CTL__ENET_RGMII_RX_CTL but with pull down */
	IOMUX_PAD(0x06A4, 0x02BC, 1, 0x0828, 1, MX6DL_ENET_PAD_CTRL_PD),

	/* AR8030 */
	/* MX6DL_PAD_ENET_RX_ER__ENET_RX_ER but with pull down */
	IOMUX_PAD(0x05C4, 0x01F4, 1, 0x0000, 0, MX6DL_ENET_PAD_CTRL_PD),	
	/* MX6DL_PAD_ENET_CRS_DV__ENET_RX_EN but with pull down */
//	IOMUX_PAD(0x05B4, 0x01E4, 1, 0x0828, 0, NO_PAD_CTRL)
	IOMUX_PAD(0x05B4, 0x01E4, 1, 0x0828, 0, MX6DL_ENET_PAD_CTRL_PD),
	/* MX6DL_PAD_ENET_RXD0__ENET_RDATA_0 but with pull down - phy addr */
	IOMUX_PAD(0x05C8, 0x01F8, 1, 0x0818, 0, MX6DL_ENET_PAD_CTRL),
	/* MX6DL_PAD_ENET_RXD1__ENET_RDATA_1 but with pull down - phy addr */
	IOMUX_PAD(0x05CC, 0x01FC, 1, 0x081C, 0, MX6DL_ENET_PAD_CTRL_PD),
...
void enet_board_init(void)
{
	...
	iomux_v3_cfg_t enet_reset =
			(_MX6Q_PAD_KEY_ROW4__GPIO_4_15 &
			~MUX_PAD_CTRL_MASK)           |
			 MUX_PAD_CTRL(0x88);
#elif defined CONFIG_MX6DL
	/* enet_reset connected to KEY_ROW4 (GPIO4, 15) */
	iomux_v3_cfg_t enet_reset = IOMUX_PAD(0x0650, 0x0268, 5, 0x0000, 0, MX6DL_ENET_PAD_CTRL_PD);
#endif

	/* Set pll8 to 25MHz first */
	REG_WR(ANATOP_BASE_ADDR, HW_ANADIG_PLL_ENET, 0x00002000);
	/* Set GPR1, bit 21 to 0 */
	mxc_iomux_set_gpr_register(1, 21, 1, 1);

	while ( 1 ){
		reg = REG_RD(ANATOP_BASE_ADDR, HW_ANADIG_PLL_ENET);
		if (reg & 0x80000000) break;
	}
	...
	/* phy reset: gpio4-15. GPIO4_BASE_ADDR is offset 0xa8000.
	   send reset twice  */
...
int checkboard(void)
{
	printf("Board: %s-C1: %s Board: 0x%x [",
	mx6_chip_name(),
	mx6_board_rev_name(),
	fsl_system_rev);
